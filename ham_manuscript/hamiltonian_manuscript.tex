...
\paragraph{Self-consistent chemical equilibrium.} Unlike standard grand canonical systems where the chemical potential $\mu$ is fixed externally, here $\beta_{ij}$ is derived self-consistently from geometry:
\begin{equation}
\beta_{ij} = \mathrm{softmax}_j\left(-\frac{\KL_{ij}}{\kappa}\right)
\end{equation}
This represents chemical equilibrium---the system determines its own coupling strengths via maximum entropy over the divergence landscape. No free parameters beyond $\kappa$ (set to 1 in natural units where 1 nat = 1 unit of information cost).

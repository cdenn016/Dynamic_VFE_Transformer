...
\paragraph{Negative chemical potential.} The coupling term $-\sum_{ij}\beta_{ij}\KL_{ij}$ can be written as $+\sum_{ij}\mu_{ij}\KL_{ij}$ where $\mu_{ij} = -\beta_{ij} < 0$. Divergence has \emph{negative chemical potential}, exhibiting bosonic statistics: like bosons that bunch into the same quantum state, agents with $\mu < 0$ tend toward alignment (shared belief state) in the absence of differentiating observations. Observations lift this degeneracy, acting as an external potential that forces differentiation---analogous to how an external field breaks Bose-Einstein condensation.
